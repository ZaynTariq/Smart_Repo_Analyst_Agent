...
/// Respects maxFiles limit and supports optional githubToken parameter for auth.
/// </summary>
public class RepoService
{
    ...
    /// Optional githubToken overrides any configured token and is used temporarily.
    /// </summary>
    public async Task<RepoContent> FetchRepositoryAsync(string repoUrl, int maxFiles = 20, string? githubToken = null)
    {
        var (owner, repo) = ParseRepoUrl(repoUrl);
        var rootApi = $"https://api.github.com/repos/{owner}/{repo}/contents";

        // Temporarily set auth header if token provided
        AuthenticationHeaderValue? previousAuth = null;
        if (!string.IsNullOrWhiteSpace(githubToken))
        {
            previousAuth = _http.DefaultRequestHeaders.Authorization;
            _http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("token", githubToken);
        }
            ...
            while (toVisit.Count > 0 && files.Count < maxFiles)
            {
                ...
                if (root.ValueKind == JsonValueKind.Array)
                {
                    foreach (var item in root.EnumerateArray())
                    {
                        ...
                        bool isTextFile = PossibleTextFileExtensions.Any(ext => name.EndsWith(ext, StringComparison.OrdinalIgnoreCase));
                        if (type == "dir")
                        {
                        ...
                        else if (type == "file")
                        {
                            ...
                            else if (isTextFile && !string.IsNullOrWhiteSpace(downloadUrl))
                            {
        ...
        finally
        {
            ...
            if (!string.IsNullOrWhiteSpace(githubToken))
            {
                _http.DefaultRequestHeaders.Authorization = previousAuth;
            }
        }
    }

    private readonly static string[] PossibleTextFileExtensions = [
            ".cs", ".csproj", ".json", ".xml", ".md", ".txt",
            ".js", ".ts", ".html", ".css",
